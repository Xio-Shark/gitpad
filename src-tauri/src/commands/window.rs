...
pub fn open_workspace(app: AppHandle, path: String) -> Result<(), AppError> {
    ...
    let url = if cfg!(dev) {
        let url_str = format!("http://localhost:1420/?path={}", url_encode(&path));
        match url_str.parse() {
            Ok(u) => WebviewUrl::External(u),
            Err(_) => WebviewUrl::App(format!("index.html?path={}", url_encode(&path)).into()),
        }
    } else {
        WebviewUrl::App(format!("index.html?path={}", url_encode(&path)).into())
    };
